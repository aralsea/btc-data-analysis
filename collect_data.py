...
def get_new_data(periods: int, length: int)-> None:
    ...
    Args:
        periods (int): 足を指定. Ex. 15分足: 900, 日足: 86400
        length (int): データの取得件数, <=6000 (一度に6000件までしか取得できない, 6000より大きい件数を返すよう指定してもAPIからは6000件しか返ってこない)
    ...
    print(f"Data from {dt.datetime.fromtimestamp(data[0][0])} to {dt.datetime.fromtimestamp(data[-1][0])} are saved")
...
def add_data(periods: int) -> None:
    ...
    before = dt.datetime.now()
    after = dt.datetime.fromtimestamp(old_df["CloseTime"][-1]) + dt.timedelta(seconds=periods)
